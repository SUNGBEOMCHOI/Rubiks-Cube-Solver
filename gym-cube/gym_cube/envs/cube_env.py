...
class CubeEnv(gym.Env):
    ...
    def init_state(self):
        ...
        if self.show_cube:
            self.render_cube = RenderCube(self.cube_size, env = self)
            # env 전달하기
    ...
    def render(self):
        self.render_cube = RenderCube(env=self, N = self.cube_size)
        self.fig = self.render_cube.draw_interactive()
        self.show_cube=True
        plt.show()
        # plt.close()
    ...
    def get_target_value(self, model, scramble_count, temperature):
        ...
        for action in range(self.action_dim):
            if self.cube_size == 2:
                ...
                # before_solved_time = time.time() - start_time
                ...
                # print("Solved and Action Time: ", solved_time - before_solved_time)
                # next_state = self.sim_state_to_state(self.sim_cube)
                # next_state_list.append(self.sim_state_to_state(self.sim_cube))
                # conversion_time = time.time() - start_time
                # print("Conversion Time: ", conversion_time - solved_time)
                # self.sim_cube.perform_step(self.action_to_sim_action[self.cube_size][counter_action])
                # counter_action_time = time.time() - start_time
                # print("Counter Action Time: ", counter_action_time - conversion_time)
    ...
    def save_video(self, cube_size, scramble_count, sample_cube_count, video_path='./video'):
        ...
        plt.figure(figsize=(frames[0].shape[1] / 36.0, frames[0].shape[0] / 36.0), dpi=36) # fig size up
